...
export default function App() {
  ...
  const toggleEmailVisibility = async () => {
    ...
    if (!error) {
      ...
      showToast(newValue ? "Email visible" : "Email hidden");
    }
  };
  ...
  return (
    <div className={`min-h-screen ${colors.bg} ${colors.text} pb-36 transition-colors`}>
      {statusMsg.text && (
        <div className={`fixed top-24 left-1/2 -translate-x-1/2 z-[10001] flex items-center gap-2 px-6 py-3 rounded-2xl border backdrop-blur-xl animate-in fade-in slide-in-from-top-4 ${statusMsg.type === 'error' ? 'bg-red-500/10 border-red-500/20 text-red-400' : 'bg-emerald-500/10 border-emerald-500/20 text-emerald-400'}`}>
      ...
      <div className="max-w-md mx-auto px-6 -mt-16 relative z-30">
        ...
        {activeTab === 'dev' && isAdmin && (
          <AdminTab 
            spots={spots} unlockedSpots={unlockedSpots} claimSpot={claimSpot} removeSpot={(id) => {
              const newUnlocked = unlockedSpots.filter(x => x !== id);
              supabase.from('profiles').update({ unlocked_spots: newUnlocked }).eq('id', user.id).then(() => setUnlockedSpots(newUnlocked));
            }} 
