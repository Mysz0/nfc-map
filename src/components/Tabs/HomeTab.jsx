...
export default function HomeTab({ isNearSpot, totalPoints, foundCount, unlockedSpots, visitData, spots, colors }) {
  ...
  return (
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-6 duration-700">
      {/* 1. PROXIMITY ALERT - Feature Preserved */}
      {isNearSpot && (
      ...
      {/* 2. STAT CARD - Feature Preserved */}
      <StatCard mainVal={totalPoints} subVal={foundCount} colors={colors} />

      {/* 3. UPDATED COLLECTIONS WITH STREAKS */}
      <div className="space-y-4">
        <h2 className="text-[9px] font-black uppercase tracking-[0.3em] text-emerald-500/50 px-4">Active Nodes & Streaks</h2>
        ...
        {(!unlockedSpots || unlockedSpots.length === 0) ? (
          ...
          unlockedSpots.map(id => {
            const spot = spots?.[id];
            // Defensive check: if spot data isn't loaded yet, don't render and crash
            if (!spot) return null;
              ...
              <div key={id} className={`group relative ${colors.card} p-5 rounded-[2.2rem] border transition-all duration-300 hover:scale-[1.02] backdrop-blur-md overflow-hidden`}>
                ...
                <div className="flex items-center justify-between relative z-10">
                  ...
                  <div className="text-right">
                    <p className="text-xs font-black tracking-tighter">+{Math.round(spot.points * parseFloat(multiplier))}</p>
                    <p className="text-[8px] text-zinc-500 font-bold uppercase tracking-widest">Yield</p>
                  </div>
                </div>

                {/* Progress Bar for Recharging */}
                <div className="mt-4 pt-4 border-t border-white/[0.05]">
                  <div className="h-1 w-full bg-white/5 rounded-full overflow-hidden">
                    <div className={`h-full transition-all duration-500 ${canCheckIn ? 'w-full bg-emerald-500' : 'w-1/2 bg-zinc-700'}`} />
                  </div>
                </div>
              </div>
            );
